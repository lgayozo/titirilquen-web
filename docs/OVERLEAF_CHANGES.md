...
| **B3** (D‑08) | Logit de uso de suelo con `λ_h` heterogéneo | **RESUELTO (2026‑06):** se implementó el logit heteroscedástico (`solve_heteroscedastic`) como corrección. **La web, sin embargo, corre siempre `logit`** (el heteroscedástico no se expone: precio en utiles; ver §C8). Derivación paper‑ready en **§C8** abajo. | Presentar la corrección en el paper como referencia teórica, aclarando que la implementación web usa `logit`; conservar `logit` como método principal (el `frechet` se quitó por redundante). |
