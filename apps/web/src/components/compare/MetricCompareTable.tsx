 ...
 */
export function MetricCompareTable({ cols, rows, baseId }: MetricCompareTableProps) {
  const { t } = useTranslation("simulator");

  const base = cols.find((c) => c.id === baseId && c.values) ?? cols.find((c) => c.values);
          ...
          {rows.map((row) => (
            ...
            >
              ...
              {cols.map((c) => {
                ...
                if (v == null) {
                  return (
                    <td key={c.id} className="px-3 py-1.5 text-right text-[var(--muted)]">
                      —
                    </td>
                  );
                }
                const baseV = base && base.id !== c.id ? base.values?.[row.key] : null;
                ...
                return (
                  <td key={c.id} className="px-3 py-1.5 text-right">
                    ...
                    {delta != null && Math.abs(delta) > 1e-9 && (
                      <span className={cn("ml-1.5", deltaColor(delta, row.betterWhen))}>
                        {delta > 0 ? "+" : ""}
